...
%In Figure \ref{cycles-vs-null} we also compared when $A^{p'} = \mathbb{1}$ to when $A$ is singular, and verified that when nullity is positive, $A$ can never be a root of the (modular) identity.
...
%\begin{figure}[H]
  %\caption{Nullity and $p'$ at various $\mathbb{Z}_k$ and grid sizes}
