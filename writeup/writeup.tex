...
\noindent that is, where after $n_1$ "iterations" of applying $A$, we would enter a repeating cycle with period $n_2$ that never returned to any members of our initial set of $n_1$ transient states. In these cases $A^n$ never equalled the identity, which clearly must be true for \eqref{eq:transient} to hold. Some examples are visible in Figures \ref{655-transient} and \ref{655-transient-long}.
